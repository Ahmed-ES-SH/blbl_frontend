...
export default function OffersPage() {
  ...
  const handledelete = async (id: string) => {
    try {
      const res = instance.delete(`/service-slider/${id}`);
  ...
  console.log(offers);
  return (
    <div className="h-[85vh] bg-main_color dark:bg-transparent rounded-md hidden-scrollbar overflow-y-auto w-full flex items-start justify-center">
      {offers == null ? (
        <LoadingDashbord />
      ) : offers.length > 0 ? (
        <div className="mt-8  overflow-y-auto  w-full px-4  flex flex-col items-start gap-4">
          {offers.map((offer: allstring, index: number) => (
            <div
              key={index}
              className="w-full relative group pl-2 py-2  overflow-hidden max-md:flex-col-reverse max-md:gap-4 max-md:items-start flex items-center justify-between rounded-md bg-secend_color dark:bg-secend_dash border shadow-md text-white dark:text-secend_color"
            >
              ...
              <div className="image w-[120px] max-md:w-full h-full">
                <Image
                  src={offer.image}
                  width={1024}
                  height={1280}
